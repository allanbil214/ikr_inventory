...
/**
 * admin/logs/list.php
 * Phase 8. Expects: $logs, $teknisiList, $workOrders, $categories,
 * $showDeleted, $flash, $user, and the raw filter values in $_GET
 * (technician, wo, category, q, date_from, date_to, show_deleted).
 */
...
<div class="container">
    ...
    <form method="GET" action="index.php" class="filter-panel">
        ...
        <div class="chip-row">
            <a href="index.php?page=logs<?= $currentTechnician !== '' ? '&technician=' . urlencode($currentTechnician) : '' ?><?= $currentWo !== '' ? '&wo=' . urlencode($currentWo) : '' ?><?= $currentSearch !== '' ? '&q=' . urlencode($currentSearch) : '' ?><?= $currentDateFrom !== '' ? '&date_from=' . urlencode($currentDateFrom) : '' ?><?= $currentDateTo !== '' ? '&date_to=' . urlencode($currentDateTo) : '' ?><?= $showDeleted ? '&show_deleted=1' : '' ?>"
               class="chip<?= $currentCategory === '' ? ' active' : '' ?>">Semua</a>
            <?php foreach ($categories as $cat): ?>
                <a href="index.php?page=logs&category=<?= $cat['id'] ?><?= $currentTechnician !== '' ? '&technician=' . urlencode($currentTechnician) : '' ?><?= $currentWo !== '' ? '&wo=' . urlencode($currentWo) : '' ?><?= $currentSearch !== '' ? '&q=' . urlencode($currentSearch) : '' ?><?= $currentDateFrom !== '' ? '&date_from=' . urlencode($currentDateFrom) : '' ?><?= $currentDateTo !== '' ? '&date_to=' . urlencode($currentDateTo) : '' ?><?= $showDeleted ? '&show_deleted=1' : '' ?>"
                ...
                <div class="serial-row log-row<?= $isDeleted ? ' log-row-deleted' : '' ?>">
                    <div class="log-row-info">
                        ...
                        <div class="material-meta">
                            <?= htmlspecialchars($log['technician_name']) ?> ·
                    ...
                    <div class="log-row-actions">
                        ...
                        <?php else: ?>
                            <a href="index.php?page=usage-log-edit&id=<?= $log['id'] ?>&return_to=logs" class="btn-small">Edit</a>
